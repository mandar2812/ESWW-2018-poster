...
The model must produce for a given input $x(t)$ at time $t$ the following predictions:
...
The proposed model architecture takes the form of a neural network model as seen above.
...
Generating time series $x(t) \in \mathbb{R}^8$, of size $4000$ (one copy for training and another for test), using \emph{Stochastic Langevin Dynamics} as shown in equation \ref{eq:data}.
...
\hline
\noalign{\smallskip}
\multicolumn{3}{c}{Predictive Performance: Time Lag} \\
\hline
Experiment  &  M.A.E  &  Correlation \\
\noalign{\smallskip}
\hline
\noalign{\smallskip}
I & $0.763$ & $0.899$ \\
...
\hline
\noalign{\smallskip}
\multicolumn{3}{c}{Predictive Performance: Output} \\
\hline
Experiment  &  M.A.E  &  Correlation \\
\noalign{\smallskip}
\hline
